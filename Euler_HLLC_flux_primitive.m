...
vn_l = dot(U_l(2:4), normal(:)); % normal speed
...
vn_r = dot(U_r(2:4), normal(:)); % normal speed
...
if S_l > 0.0
    ...
    Fn(1) = vn_l * rho_l;
    Fn(2:4) = Fn(1) * v_l(:) + p_l * normal(:);
    Fn(5) = vn_l * (E_l + p_l);
elseif S_r < 0.0
    % supersonic flow toward left
    Fn(1) = vn_r * rho_r;
    Fn(2:4) = Fn(1) * v_r(:) + p_r * normal(:);
    Fn(5) = vn_r * (E_r + p_r);
else
    ...
    if S_m >= 0.0
        ...
        vv_l = vn_l - ts_l;
        Fn(1) = vv_l * rho_l;
        Fn(2:4) = Fn(1) * v_l(:) + (p_l + tsflow_l) * normal(:);
        Fn(5) = vv_l * (E_l + p_l) + tsflow_l * S_m;
    else
        ...
        vv_r = vn_r - ts_r;
        Fn(1) = vv_r * rho_r;
        Fn(2:4) = Fn(1) * v_r(:) + (p_r + tsflow_r) * normal(:);
        Fn(5) = vv_r * (E_r + p_r) + tsflow_r * S_m;
    end
end
